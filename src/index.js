...
async function run() {
  try {
    ...
    const messageMissing = core.getInput("reminder-message", {
      required: true,
    });
      ...
      if (!hasLog) {
        ...
        for (const url of logUrls) {
          ...
          try {
            ...
            if (parsedLog.plugins.isLoaded) isModded = true;
            ...
            core.warning(
              `Unable to download some of the logs, results may be incomplete: ${e.message}`,
            );
          }
        }
...
function formatMarkdownMessage(data) {
  function resultsTable(res) {
    ...
    const rows = res.map((r) => [
      ...
      r.headset,
      r.plugins.isLoaded
        ? `Yes ${r.plugins.modLoader === null ? "" : `(${r.plugins.modLoader})`}`
        : "no",
      r.renderer.name,
      r.cleanExit ? "✅" : "❌",
    ]);
